...
class ExpDataLoader:
    def __init__(self, data_dir):
        with open(os.path.join(data_dir, "exp_splits.pkl"), "rb") as f:
            self.exp_data = pickle.load(f)
...
class SeqDataLoader:
    def __init__(self, data_dir):
        ...
        with open(os.path.join(data_dir, "sequential_data.txt"), "r") as f:
            for line in f.readlines():
                user, items = line.strip().split(" ", 1)
                self.user2items_positive[int(user)] = items.split(" ")

        self.user2items_negative = {}
        with open(os.path.join(data_dir, "negative_samples.txt"), "r") as f:
            for line in f.readlines():
                user, items = line.strip().split(" ", 1)
                self.user2items_negative[int(user)] = items.split(" ")

        with open(os.path.join(data_dir, "datamaps.json"), "r") as f:
...
def compute_whole_word_id(seq_batch, tokenizer, max_len):
    ...
    for seq in seq_batch:
        ...
        for i, (start, end) in enumerate(zip(start_indices, end_indices)):
            whole_word_id[start:end] = [i + 1] * (
                end - start
            )  # leave 0 as padding token
    ...
    for whole_word_id in whole_word_ids:
        padded_whole_word_ids.append(
            whole_word_id + [0] * (max_len - len(whole_word_id))
        )
    return padded_whole_word_ids


class ExpSampler:
    def __init__(self, exp_data, data_dir):
        self.task_id = 0
        ...
        with open(os.path.join(data_dir, "datamaps.json"), "r") as f:
            datamaps = json.load(f)
        self.user_to_id = datamaps["user2id"]
    ...
    def sample(self, num):
        ...
        for _ in range(num):
            ...
            user_id = self.user_to_id[record["reviewerID"]]
            inputs.append(template.format(user_id, record["asin"]))
            outputs.append(record["explanation"])
            users.append(user_id)
            self.step += 1
        return task, users, inputs, outputs
...
class SeqSampler:
    ...
    def sample_seq(self, u):
        ...
        end_item = random.randint(
            start_item + 1, len(item_history) - 3
        )  # cannot be the last 2
        item_seg = item_history[
            start_item : (end_item + 1)
        ]  # sample a segment from the sequence without the last two
...
class TrainBatchify:
    def __init__(
        ...
        self.exp_sampler = ExpSampler(exp_data, data_dir)
    ...
    def encode(
        ...
        users = [str(u) for u in users]  # convert to int first
        return task, users, source_seq, source_mask, whole_word, target_seq

    def next_batch(self):
        self.batch_index += 1
        if self.batch_index % 3 == 1:
            task_list, user_list, input_list, output_list = self.exp_sampler.sample(
                self.batch_size
            )
        elif self.batch_index % 3 == 2:
            task_list, user_list, input_list, output_list = self.seq_sampler.sample(
                self.batch_size
            )
        else:
            task_list, user_list, input_list, output_list = self.topn_sampler.sample(
                self.batch_size
            )
        return self.encode(task_list, user_list, input_list, output_list)


class ExpBatchify:
    def __init__(self, exp_data, user2id, tokenizer, exp_len, batch_size):
        self.task_id = 0
        ...
        for x in exp_data:
            input_list.append(template.format(self.user2id[x["reviewerID"]], x["asin"]))
            output_list.append(x["explanation"])
            user_ids.append(self.user2id[x["reviewerID"]])
        encoded_source = tokenizer(input_list, padding=True, return_tensors="pt")
        self.users = [str(u) for u in user_ids]
...
class SeqBatchify:
    ...
    def encode(self, input_list, user_list, output_list):
        ...
        whole_word = torch.tensor(whole_word_ids, dtype=torch.int64).contiguous()
        # encode users
        users = [str(u) for u in user_list]

        encoded_target = self.tokenizer(output_list, padding=True, return_tensors="pt")
        target_seq = encoded_target["input_ids"]
        task = torch.ones((sample_num,), dtype=torch.int64) * self.task_id
        return task, user_list, source_seq, source_mask, whole_word, target_seq
    ...
    def next_batch(self, valid=True):
        ...
        for i in range(start, offset):
            ...
            if len(item_seg) > self.max_seq_len:
                item_seg = item_seg[-self.max_seq_len :]
            input_seq = self.user_template.format(
                u, self.item_template.join(item_seg[:-1])
            )
            # input_seq = 'user_{}'.format(u)
            input_list.append(input_seq)
            user_list.append(int(u))
            output_list.append(item_seg[-1])
        return self.encode(input_list, user_list, output_list)
...
class TopNBatchify:
    ...
    def sample_positive(self, user, anchor):
        items_pos = self.user2item_set_pos[user]
        if len(items_pos) < 2:
            return None
        while True:
            i = random.randint(1, self.item_num)
            if i in items_pos and i != anchor:
                return str(i)
    ...
    def next_batch(self, valid=True):
        ...
        for i in range(start, offset):
            ...
            random.shuffle(item_list_neg)
            input_seq = self.user_template.format(
                u, self.item_template.join(item_list_neg)
            )
            input_list.append(input_seq)
            user_list.append(int(u))
